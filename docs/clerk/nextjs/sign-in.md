# Source: https://clerk.com/docs/components/authentication/sign-in
# Last fetched: 2026-03-27T18:38:37.271521+00:00

#

# <SignIn /> component

Clerk's <SignIn /> component renders a UI for signing in users.

This page is available for the following SDKs. Select the SDK you would like to use:

### [Next.js](/docs/nextjs/reference/components/authentication/sign-in)

### [React](/docs/react/reference/components/authentication/sign-in)

### [Expo](/docs/expo/reference/components/authentication/sign-in)

![](data:image/webp;base64,)

### [TanStack React Start](/docs/tanstack-react-start/reference/components/authentication/sign-in)

### [React Router](/docs/react-router/reference/components/authentication/sign-in)

### [Astro](/docs/astro/reference/components/authentication/sign-in)

### [Chrome Extension](/docs/chrome-extension/reference/components/authentication/sign-in)

### [JavaScript](/docs/js-frontend/reference/components/authentication/sign-in)

### [Nuxt](/docs/nuxt/reference/components/authentication/sign-in)

### [Vue](/docs/vue/reference/components/authentication/sign-in)

## Feedback

Last updated on Mar 27, 2026

[Edit on GitHub](https://github.com/clerk/clerk-docs/edit/main/docs/reference/components/authentication/sign-in.mdx)
