# Source: https://clerk.com/docs/components/user/user-button
# Last fetched: 2026-03-27T18:38:37.271521+00:00

#

# <UserButton /> component

Clerk's <UserButton /> component is used to render the familiar user button UI popularized by Google.

This page is available for the following SDKs. Select the SDK you would like to use:

### [Next.js](/docs/nextjs/reference/components/user/user-button)

### [React](/docs/react/reference/components/user/user-button)

### [Expo](/docs/expo/reference/components/user/user-button)

![](data:image/webp;base64,)

### [TanStack React Start](/docs/tanstack-react-start/reference/components/user/user-button)

### [React Router](/docs/react-router/reference/components/user/user-button)

### [Astro](/docs/astro/reference/components/user/user-button)

### [Chrome Extension](/docs/chrome-extension/reference/components/user/user-button)

### [JavaScript](/docs/js-frontend/reference/components/user/user-button)

### [Nuxt](/docs/nuxt/reference/components/user/user-button)

### [Vue](/docs/vue/reference/components/user/user-button)

## Feedback

Last updated on Mar 27, 2026

[Edit on GitHub](https://github.com/clerk/clerk-docs/edit/main/docs/reference/components/user/user-button.mdx)
