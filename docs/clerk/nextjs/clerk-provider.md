# Source: https://clerk.com/docs/components/clerk-provider
# Last fetched: 2026-03-27T18:38:37.271521+00:00

#

# <ClerkProvider>

The <ClerkProvider> component provides session and user context to Clerk's hooks and components.

This page is available for the following SDKs. Select the SDK you would like to use:

### [Next.js](/docs/nextjs/reference/components/clerk-provider)

### [React](/docs/react/reference/components/clerk-provider)

### [Expo](/docs/expo/reference/components/clerk-provider)

![](data:image/webp;base64,)

### [TanStack React Start](/docs/tanstack-react-start/reference/components/clerk-provider)

### [React Router](/docs/react-router/reference/components/clerk-provider)

### [Chrome Extension](/docs/chrome-extension/reference/components/clerk-provider)

## Feedback

Last updated on Mar 27, 2026

[Edit on GitHub](https://github.com/clerk/clerk-docs/edit/main/docs/reference/components/clerk-provider.mdx)
