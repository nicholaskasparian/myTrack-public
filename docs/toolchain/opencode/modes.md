...
Letâs look at these configuration options in detail.
...
Control the randomness and creativity of the AIâs responses with the `temperature` config. Lower values make responses more focused and deterministic, while higher values increase creativity and variability.
...
Specify a custom system prompt file for this mode with the `prompt` config. The prompt file should contain instructions specific to the modeâs purpose.
